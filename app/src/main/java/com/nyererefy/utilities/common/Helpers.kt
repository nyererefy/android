...
fun openKonektediProfile(id: Int, mContext: Context) {
    val intent = mContext
            .packageManager
            .getLaunchIntentForPackage("com.nyererefy.nyererefy.user.profile.UserProfile")

    if (intent != null) {
        intent.putExtra(ID, id)
//        startActivity(intent)
    } else {
        Toast.makeText(mContext, "You need to Install nyererefy App", Toast.LENGTH_SHORT).show()
    }
}
